                                 ...
                                 sdf::ElementPtr _sdf)
{
  ...
  // {
    ...
    this->odometry_pub = this->rosNode->advertise<nav_msgs::Odometry>(
      this->model->GetName() + "/odometry", 10);
    this->pose_pub = this->rosNode->advertise<geometry_msgs::PoseStamped>(
      this->model->GetName() + "/pose", 10);
...
void DRCVehicleROSPlugin::RosPublishStates()
{
      ...
      this->rosPublishPeriod)
  {
    ...
    // get and publish vehicle odometry
    math::Pose vehicle_pose = this->model->GetWorldPose();
    math::Vector3 ang_vel = this->model->GetWorldAngularVel();
    math::Vector3 lin_vel = this->model->GetWorldLinearVel();

    nav_msgs::Odometry odometry;
    if (this->odometry_pub.getNumSubscribers() > 0) {
      odometry.child_frame_id = "atv/odometry";
      odometry.header.frame_id = "world";
      odometry.header.seq = this->odometry_sequence_++;
      odometry.header.stamp.sec = (this->world->GetSimTime()).sec;
      odometry.header.stamp.nsec = (this->world->GetSimTime()).nsec;

      odometry.pose.pose.position.x = vehicle_pose.pos.x;
      odometry.pose.pose.position.y = vehicle_pose.pos.y;
      odometry.pose.pose.position.z = vehicle_pose.pos.z;
      odometry.pose.pose.orientation.x = vehicle_pose.rot.x;
      odometry.pose.pose.orientation.y = vehicle_pose.rot.y;
      odometry.pose.pose.orientation.z = vehicle_pose.rot.z;
      odometry.pose.pose.orientation.w = vehicle_pose.rot.w;

      odometry.twist.twist.linear.x = lin_vel.x;
      odometry.twist.twist.linear.y = lin_vel.y;
      odometry.twist.twist.linear.z = lin_vel.z;
    ...
    if (this->pose_pub.getNumSubscribers() > 0) {
      geometry_msgs::PoseStampedPtr pose(new geometry_msgs::PoseStamped);

      pose->header.frame_id = "world";
      pose->header.seq = this->odometry_sequence_++;
      pose->header.stamp.sec = (this->world->GetSimTime()).sec;
      pose->header.stamp.nsec = (this->world->GetSimTime()).nsec;

      pose->pose.position.x = vehicle_pose.pos.x;
      pose->pose.position.y = vehicle_pose.pos.y;
      pose->pose.position.z = vehicle_pose.pos.z;
      pose->pose.orientation.x = vehicle_pose.rot.x;
      pose->pose.orientation.y = vehicle_pose.rot.y;
      pose->pose.orientation.z = vehicle_pose.rot.z;
      pose->pose.orientation.w = vehicle_pose.rot.w;
