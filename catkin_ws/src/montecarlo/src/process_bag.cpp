...
int main(int argc, const char **argv)
{
  ...
  // parser.addArgument("-d", "--directory", 1);     // directory to save frames in, e.g., 'frames'
  // parser.addArgument("-p", "--prepend", 1);       // prepend string to filename, e.g., 'img_'
  // parser.addArgument("-s", "--secs", 1);          // how many seconds of duration to run rosbag for
  // parser.addArgument("-t", "--topic", 1, false);  // topic to extract into frames (required)
  // parser.addFinalArgument("bag");
  ...
  std::string dir = ".";
  std::string prepend = "";
  // if (parser.count("directory") > 0) dir = parser.retrieve<std::string>("directory");
  // if (parser.count("prepend") > 0) prepend = parser.retrieve<std::string>("prepend");

  // parsing for duration to extract from rosbag (0 signifies infinite)
  int duration = 0;
  // if (parser.count("secs") > 0) duration = std::stoi(parser.retrieve<std::string>("secs"));
